...
const Plane = ({ flight, funcPullSeat }) => {
  const [seating, setSeating] = useState([]);
  const [selectedSeat, setSelectedSeat] = useState(null);
  const seatArr = seating?.data?.seats;
              ...
              {seat.isAvailable ? (
                <>
                  <Seat
                    ...
                    name="seat"
                    value={seat.id}
                    checked={seat.isAvailable === false}
                    onChange={() => {
                      handleChange({ id: seat.id });
                    }}
                  />
                  <Available selected={selectedSeat === seat.id}>
                    {seat.id}
                  </Available>
                </>
              ) : (
